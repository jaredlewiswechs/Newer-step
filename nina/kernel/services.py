...
class ServiceRegistry:
    ...
    def __init__(self):
        ...
        self._by_accepts: Dict[ObjectType, List[str]] = {t: [] for t in ObjectType}
...
def link_objects_service(
    obj: FoghornObject, target_hash: str = ""
) -> List[FoghornObject]:
...
def execute_service(service_name: str, obj: FoghornObject, **kwargs) -> ServiceResult:
    ...
    if not svc.can_accept(obj):
        raise ValueError(
            f"Service '{service_name}' does not accept {obj.object_type.value}"
        )

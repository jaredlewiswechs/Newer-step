...
class Inspector:
    ...
    def inspect_multiple(self, objects: List[FoghornObject]) -> InspectorData:
        ...
        if common_services:
            data.actions = [
                {"name": name, "available": True} for name in sorted(common_services)
            ]
            ...
            sections=[
                InspectorSection(
                    ...
                    fields={
                        ...
                        "Previous": (
                            obj.prev_hash[:16] + "..." if obj.prev_hash else "genesis"
                        ),
                ...
                (
                    InspectorSection(
                        ...
                        icon="📚",
                        fields={
                            f"Step {i+1}": info for i, info in enumerate(chain_info)
                        },
                        collapsed=True,
                    )
                    if chain_info

...
class BezierCurve:
    ...
    def _auto_control_points(self):
        ...
        if abs(dx) > abs(dy):
            self.control1 = Point(self.start.x + dx * 0.33, self.start.y)
            self.control2 = Point(self.end.x - dx * 0.33, self.end.y)
        else:
            # Mostly vertical, use horizontal control offsets
            self.control1 = Point(self.start.x, self.start.y + dy * 0.33)
            self.control2 = Point(self.end.x, self.end.y - dy * 0.33)
...
class CurveFactory:
    ...
    def create(
        ...
        style, color = cls.STYLE_MAP.get(
            relationship, (RelationshipStyle.SOLID, "#666666")
        )
...
def render_curves_svg(
    curves: List[BezierCurve], width: int = 800, height: int = 600
) -> str:
    """

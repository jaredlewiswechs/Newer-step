...
class FoghornObject(ABC):
    ...
    def _compute_hash(self):
        ...
        full_content = (
            f"{self.object_type.value}:{content}:{self.prev_hash or 'genesis'}"
        )
    ...
    def get_inspector_data(self) -> Dict[str, Any]:
        ...
        return {
            "general": {
                ...
                "Prev Hash": (
                    self.prev_hash[:16] + "..." if self.prev_hash else "genesis"
                ),
            },
            "metadata": self.metadata,
    ...
    def _get_inspector_sections(self) -> Dict[str, Dict]:
        return {
            ...
            "stats": {
                ...
                "Last Run": (
                    datetime.fromtimestamp(self.last_run / 1000).isoformat()
                    if self.last_run
                    else "Never"
                ),
            },

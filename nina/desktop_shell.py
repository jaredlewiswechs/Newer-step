...
class DesktopShell:
    ...
    def __init__(self, dock_items: Optional[List[str]] = None, width: int = 1024, height: int = 768):
    ...
    def snapshot(self) -> Dict[str, Any]:
        return {
            ...
            'windows': [
                {
                    'title': win.title,
                    'frame': (win.frame.x, win.frame.y, win.frame.width, win.frame.height),
                } for win in self._windows
            ],
        }
    ...
    def restore(cls, data: Dict[str, Any]) -> 'DesktopShell':
        ds = cls(dock_items=data.get('dock', []), width=data.get('width', 1024), height=data.get('height', 768))
        for winfo in data.get('windows', []):
            x, y, w, h = winfo.get('frame', (0, 0, 480, 320))
            win = NSWindow(content_rect=NSRect(x, y, w, h), title=winfo.get('title', 'Window'))
            ds.open_window(win)
        return ds
    ...
    def render_to_svg(self, width: Optional[int] = None, height: Optional[int] = None) -> str:
        ...
        for win in list(self._windows):
            ...
            parts.append(f'<g transform="translate({tx},{ty})" data-window-id="{id(win)}" data-window-title="{win.title}">')
            parts.append(inner)
            parts.append('</g>')

        # dock
        dock_h = 64
        parts.append(f'<rect x="0" y="{h - dock_h}" width="{w}" height="{dock_h}" fill="#222" opacity="0.9"/>')
        ...
        for i, win in enumerate(self._windows):
            ...
            parts.append(f'<g class="dock-item" transform="translate({cx},{y_center})">')
            parts.append(f'<circle cx="0" cy="0" r="20" fill="#fff" opacity="0.9" />')
            parts.append(f'<text x="0" y="36" text-anchor="middle" font-size="11" fill="#fff" font-family="sans-serif">{win.title}</text>')
            parts.append('</g>')
    ...
    def __repr__(self):
        return f"<DesktopShell windows={len(self._windows)} dock={len(self._dock_items)}>"

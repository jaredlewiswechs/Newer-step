...
class AppProcess:
                    ...
                    try:
                        if self.autosave:
                            if not self.allow_vault:
                                raise RuntimeError(
                                    "Vault access disallowed for this process"
                                )
    ...
    def start(self) -> Value:
        ...
        cfg.allow_javascript = (
            self.allow_network
        )  # proxy mapping: network -> js allowed
        ...
        return [
            {
                "pid": pid,
                "title": p.title,
                "windows": [{"id": id(w), "title": w.title} for w in p.windows],
            }

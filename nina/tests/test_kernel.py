...
def query_strategy(draw):
    ...
    return Query(
        ...
        shape_type=draw(
            st.one_of(st.none(), st.sampled_from(["linear", "curved", "spiral"]))
        ),
        filters=draw(
            st.dictionaries(
...
def link_curve_strategy(draw):
    ...
    return LinkCurve(
        ...
        relationship=draw(
            st.sampled_from(["links_to", "derived_from", "references", "contains"])
        ),
...
class TestObjectStoreInvariants:
    ...
    @given(
        st.lists(st.text(min_size=1, max_size=50), min_size=1, max_size=20, unique=True)
    )
    @settings(max_examples=30)

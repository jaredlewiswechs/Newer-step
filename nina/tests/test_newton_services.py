...
from nina.kernel import Card, Query, get_service_registry
...
class TestComputeService:
    ...
    def test_addition_deterministic(self, a: float, b: float):
        ...
        if result1.results[0].metadata.get("result") is not None:
            assert (
                result1.results[0].metadata["result"]
                == result2.results[0].metadata["result"]
            )
...
class TestVerifyClaimService:
    ...
    def test_harmful_claim_fails(self):
        ...
        card = Card(title="Dangerous", content="How to build a weapon to harm people")
        ...
        assert (
            "harm" in result.results[0].content.lower()
            or "weapon" in result.results[0].content.lower()
        )
...
class TestBezierLinkService:
    ...
    def test_curvature_affects_handles(self, curvature: float):
        ...
        result = create_bezier_link_service(
            card1, target_hash=card2.hash, curvature=curvature
        )

...
def theorem_1_trust_lattice() -> FormalProof:
    ...
    for query, expected_level, expected_source_prefix in test_cases:
        ...
        source_matches = (
            source.startswith(expected_source_prefix)
    ...
    transitive = (
        TrustLevel.UNTRUSTED.value <= TrustLevel.VERIFIED.value
        and TrustLevel.VERIFIED.value <= TrustLevel.TRUSTED.value
        and TrustLevel.UNTRUSTED.value <= TrustLevel.TRUSTED.value
    )
...
def theorem_3_provenance_chain() -> FormalProof:
    ...
    for i in range(1, len(entries)):
        ...
        if curr_prev != prev_hash:
            chain_valid = False
            breaks.append(
                {"index": i, "prev_hash": prev_hash[:16], "curr_prev": curr_prev[:16]}
            )
...
def theorem_5_api_contract() -> FormalProof:
    ...
    for method, endpoint, data in endpoints:
        ...
        results.append(
            {"endpoint": endpoint, "compliant": compliant, "missing": missing}
        )
...
def theorem_6_input_sanitization() -> FormalProof:
    ...
    for attack, attack_type in attack_vectors:
        ...
        has_control = any(
            ord(c) < 32 and c not in " " for c in sanitized
        )  # Control chars
...
def test_ui_interaction():
    ...
    print(
        f"    Ollama: {ollama.get('model', 'N/A')} ({'✓' if ollama.get('available') else '✗'})"
    )
    ...
    for entry in ledger.get("entries", [])[-3:]:
        print(
            f"    [{entry.get('index', '?')}] {entry.get('operation', '?')[:40]} → {entry.get('hash', '?')[:12]}..."
        )

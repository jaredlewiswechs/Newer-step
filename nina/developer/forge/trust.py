...
class TrustLattice:
        ...
        self._upgrade_log.append(
            {
                ...
                "value_hash": hashlib.sha256(str(before.value).encode()).hexdigest()[
                    :16
                ],
    ...
    def combine(
        self, *labeled_values: Labeled, combiner: Callable[..., T]
    ) -> Labeled[T]:
        """
    ...
    def require_trust(self, labeled: Labeled[T], minimum_trust: TrustLabel) -> T:
        """
    ...
    def verify_with(
        ...
        return self.upgrade(
            labeled, self._verifiers[verifier_name], target_trust, verifier_name
        )
...
if __name__ == "__main__":
    ...
    try:
        trusted_city = lattice.upgrade(
            city_input, is_valid_city, verifier_name="city_check"
        )
    ...
    for entry in log:
        print(
            f"   {entry['verifier']}: {entry['before_trust']} → {entry['after_trust']} "

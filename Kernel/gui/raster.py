...
def rasterize_to_png_bytes(
    path, width: int = 400, height: int = 400, background: Optional[str] = None
) -> bytes:
    try:
        import cairosvg
    except Exception as e:
        raise ImportError(
            "cairosvg is required to rasterize paths to PNG. Install with `pip install cairosvg`."
        ) from e

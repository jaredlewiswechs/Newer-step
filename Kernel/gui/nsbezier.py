...
class NSBezierPath:
    ...
    def curve_to_point(self, control1: NSPoint, control2: NSPoint, end: NSPoint):
        # cubic bezier
        self._cmds.append(
            _Cmd("C", (control1.x, control1.y, control2.x, control2.y, end.x, end.y))
        )
    ...
    def append_oval_in_rect(self, x: float, y: float, w: float, h: float):
        ...
        self.curve_to_point(
            NSPoint(cx + rx, cy + k * ry),
            NSPoint(cx + k * rx, cy + ry),
            NSPoint(cx, cy + ry),
        )
        self.curve_to_point(
            NSPoint(cx - k * rx, cy + ry),
    ...
    def element_at_index(self, i: int):
        ...
        if c.name == "C":
            return (
                self.NSCurveToBezierPathElement,
                [
                    NSPoint(c.args[0], c.args[1]),
                    NSPoint(c.args[2], c.args[3]),
                    NSPoint(c.args[4], c.args[5]),
                ],
            )
        if c.name == "Z":
        ...
        if c.name == "A":
            ...
            return (
                self.NSArcBezierPathElement,
                [NSPoint(c.args[6], c.args[7]), c.args[8], c.args[9]],
            )
        return (None, [])
    ...
    def bounds(self):
        ...
        for c in self._cmds:
            if c.name in ("M", "L"):
                ...
                add_point(c.args[0], c.args[1])
                add_point(c.args[2], c.args[3])
                add_point(c.args[4], c.args[5])
            elif c.name == "A":
                # sample arc
    ...
    def relative_curve_to_point(
        ...
        if cur is None:
            ...
            self.curve_to_point(
                NSPoint(control1_dx, control1_dy),
                NSPoint(control2_dx, control2_dy),
                NSPoint(end_dx, end_dy),
            )
        else:
            x0, y0 = cur
            self.curve_to_point(
                NSPoint(x0 + control1_dx, y0 + control1_dy),
                NSPoint(x0 + control2_dx, y0 + control2_dy),
                NSPoint(x0 + end_dx, y0 + end_dy),
            )
    ...
    def append_rounded_rect(self, x: float, y: float, w: float, h: float, r: float):
        ...
        cy = y + r
        self.curve_to_point(
            NSPoint(cx - r, cy - k * r), NSPoint(cx - k * r, cy - r), NSPoint(x + r, y)
        )
        self.close_path()
    ...
    def _flattened_segments(self, curve_steps=12, tolerance: float = 0.5):
        ...
        for c in self._cmds:
            if c.name == "M":
                ...
                segs.extend(
                    subdivide_cubic((x0, y0), (x1, y1), (x2, y2), (x3, y3), tolerance)
                )
                cur = (x3, y3)
            elif c.name == "A":
                rx, ry, rot, laf, sf, x, y, cx, cy, start, end = c.args
    ...
    # Transforms
    def transform(
        self, a: float, b: float, c: float, d: float, tx: float = 0.0, ty: float = 0.0
    ):
        def apply(x, y):
        ...
        self._cmds.append(
            _Cmd(
                ...
                (
                    ...
                    r,
                    0,
                    1 if abs(end - start) > math.pi else 0,
    ...
    def append_arc_from_point_to_point_radius(
        self, from_pt: NSPoint, to_pt: NSPoint, radius: float
    ):
        ...
        self._cmds.append(
            _Cmd(
                ...
                (
                    ...
                    radius,
                    0,
                    1 if abs(end_ang - start_ang) > math.pi else 0,
                    0 if clockwise else 1,
                    t2[0],
                    t2[1],
                    cx,
    ...
    def append_bezier_path_with_arc_from_point_to_point_radius(
        self, from_pt: NSPoint, to_pt: NSPoint, radius: float
    ):
    ...
    def append_arc_with_center_degrees(
        self,
        center: NSPoint,
        radius: float,
        start_deg: float,
        ...
        return self.append_arc_with_center(
            center, radius, math.radians(start_deg), math.radians(end_deg), clockwise
        )
        ...
        for sp in subpaths:
            ...
            for e in rev_iter:
                ...
                if e.name == "L":
                    ...
                    new._cmds.append(
                        _Cmd("A", (rx, ry, rot, laf, new_sweep, x, y, cx, cy, eang, s))
                    )
                elif e.name == "Z":
                    new.close_path()
        return new
    ...
    def _to_path_d(self) -> str:
        ...
        for c in self._cmds:
            if c.name == "M" or c.name == "L":
                ...
                parts.append(
                    f"C {c.args[0]} {c.args[1]} {c.args[2]} {c.args[3]} {c.args[4]} {c.args[5]}"
                )
            elif c.name == "Z":
                parts.append("Z")
            elif c.name == "A":
    ...
    def to_svg(self, width: float = 400, height: float = 400) -> str:
        ...
        if self._dash_pattern:
            dash_attr = (
                f' stroke-dasharray="{" ".join(str(x) for x in self._dash_pattern)}"'
            )

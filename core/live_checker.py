...
def count_citations(text: str) -> int:
    ...
    matches = re.findall(
        r"\(.*?\d{4}.*?\)|et al\.|doi:|https?://", text or "", flags=re.I
    )
    return len(matches)
...
def compute_envelope_distance(omega: Omega, draft_text: str) -> Dict[str, Any]:
    ...
    wc_score = bounded_normalized_distance(
        wc, omega.word_count_min, omega.word_count_max
    )
    cit_score = citation_score(
        omega.citations.required, cits, omega.citations.min_count
    )
    ...
    if wc_score < 1.0:
        if omega.word_count_min and wc < omega.word_count_min:
            suggestions.append(
                f"Increase word count to at least {omega.word_count_min} words"
            )
        elif omega.word_count_max and wc > omega.word_count_max:
            suggestions.append(
                f"Reduce word count to at most {omega.word_count_max} words"
            )
    if cit_score < 1.0:
        suggestions.append(
            "Add more citations to meet the assignment's citation requirement"
        )
    if struct_score < 1.0:
        suggestions.append(
            "Include required section headings or labels listed in the assignment structure"
        )

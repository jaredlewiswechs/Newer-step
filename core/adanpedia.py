...
_MOCK_CORPUS = [
    {
        "id": "w1",
        "title": "Example: Intro + Background",
        "handle": "Introduction: context and thesis",
        "snippet": "[An admissible example intro skeleton]",
    },
    {
        "id": "w2",
        "title": "Example: Thematic approach",
        "handle": "Theme 1: definition & scope",
        "snippet": "[A thematic trajectory sample]",
    },
    {
        "id": "w3",
        "title": "Example: Chronological arc",
        "handle": "Early developments / background",
        "snippet": "[Chronological handle sample]",
    },
]


def _from_knowledge_store(
    handles: Optional[List[str]] = None, max_examples: int = 3
) -> List[Dict]:
    try:
        ...
        if not handles:
            ...
            for f in facts[:max_examples]:
                results.append(
                    {"id": f.id, "title": f.key, "handle": f.key, "snippet": f.fact}
                )
            return results

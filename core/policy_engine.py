...
class PolicyEngine:
    ...
    def evaluate_input(
        self, input_data: Any, operation: str
    ...
    def evaluate_output(
        self, output_data: Any, operation: str
    ) -> List[PolicyEvaluationResult]:
        """
    ...
    def _evaluate_policy(
        self, policy: Policy, data: Any, operation: str
        ...
        try:
            if policy.type == PolicyType.SIZE_LIMIT:
                ...
                if not passed:
                    message = (
                        f"Size limit exceeded: {condition.get('max_size', 'unknown')}"
                    )
        ...
        return PolicyEvaluationResult(
            policy_id=policy.id, passed=passed, action=policy.action, message=message
        )
    ...
    def check_enforcement_needed(self, results: List[PolicyEvaluationResult]) -> bool:
        ...
        for result in results:
            if not result.passed and result.action in [
                PolicyAction.DENY,
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "violation_rate": (
                round(self._violation_count / self._evaluation_count * 100, 2)

...
from core.frame_generator import (
    generate_handles_from_omega,
    generate_handles_from_prompt,
)
...
async def frames(req: FrameRequest, use_llm: bool = False) -> Dict[str, Any]:
    if use_llm:
        if not (_HAS_OLLAMA_FRAME or _HAS_OLLAMA_EXTRACTOR):
            raise HTTPException(
                status_code=503, detail="LLM unavailable for frame generation"
            )
        # Prefer the dedicated frame suggester when available
        if _HAS_OLLAMA_FRAME:
            try:
                handles = llm_suggest_handles(
                    req.prompt or (req.omega.prompt if req.omega else "")
                )
            except Exception as e:
                raise HTTPException(
                    status_code=500, detail=f"LLM frame suggestion failed: {e}"
                )
        else:
            # Fallback: use extractor + simple template mapping (not implemented deeply here)
            handles = generate_handles_from_prompt(
                req.prompt or (req.omega.prompt if req.omega else "")
            )
    else:
        if req.omega is not None:
            ...
            raise HTTPException(
                status_code=400, detail="Provide `prompt` or `omega` in request body"
            )

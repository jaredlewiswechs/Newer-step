...
def import_python(
    module_name: str, names: Optional[List[str]] = None
) -> Dict[str, Value]:
    ...
    if module_name not in _ffi_config.trusted_modules:
        if not any(
            module_name.startswith(m + ".") for m in _ffi_config.trusted_modules
        ):
...
"""
    ...
    try:
        ...
        result = subprocess.run(
            ["node", temp_path], capture_output=True, text=True, timeout=30
        )
...
"""
    ...
    try:
        ...
        result = subprocess.run(
            ["go", "run", temp_path], capture_output=True, text=True, timeout=30
        )
...
def import_external(
    module: str, scope, names: Optional[List[str]] = None, alias: Optional[str] = None
):

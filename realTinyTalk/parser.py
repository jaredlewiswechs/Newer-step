...
class Parser:
    ...
    def _expect_end(self):
        ...
        if not self._check(
            TokenType.NEWLINE, TokenType.EOF, TokenType.RBRACE, TokenType.SEMICOLON
        ):
    ...
    def _parse_const(self) -> ConstStmt:
        ...
        return ConstStmt(
            name=name_tok.value, value=value, line=tok.line, column=tok.column
        )
    ...
    def _parse_struct(self) -> StructDecl:
        ...
        return StructDecl(
            name=name_tok.value, fields=fields, line=tok.line, column=tok.column
        )
    ...
    def _parse_enum(self) -> EnumDecl:
        ...
        return EnumDecl(
            name=name_tok.value, variants=variants, line=tok.line, column=tok.column
        )
    ...
    def _parse_try(self) -> TryStmt:
        ...
        if self._match(TokenType.CATCH):
            if self._match(TokenType.LPAREN):
                catch_var_tok = self._consume(
                    TokenType.IDENTIFIER, "Expected catch variable"
                )
    ...
    def _parse_when(self):
        ...
        if self._match(TokenType.LPAREN):
            ...
            statements = []
            while (
                not self._check(TokenType.FINFR, TokenType.END) and not self._at_end()
            ):
                stmt = self._parse_statement()
                if stmt:
                    statements.append(stmt)
            ...
            if not self._match(TokenType.FINFR):
                self._consume(
                    TokenType.END, "Expected 'finfr' or 'end' after when function"
                )
    ...
    def _parse_expression_statement(self) -> ASTNode:
        ...
        if self._match(
            ...
            value = self._parse_expression()
            return AssignStmt(
                target=expr, value=value, op=op, line=expr.line, column=expr.column
            )
    ...
    def _parse_or(self) -> ASTNode:
        ...
        while self._match(TokenType.OR):
            ...
            left = BinaryOp(
                op="or", left=left, right=right, line=tok.line, column=tok.column
            )
    ...
    def _parse_and(self) -> ASTNode:
        ...
        while self._match(TokenType.AND):
            ...
            left = BinaryOp(
                op="and", left=left, right=right, line=tok.line, column=tok.column
            )
    ...
    def _parse_equality(self) -> ASTNode:
        ...
        while self._match(
            ...
            op_map = {
                ...
                TokenType.ISNT: "isnt",
                TokenType.HAS: "has",
                TokenType.HASNT: "hasnt",
                TokenType.ISIN: "isin",
                TokenType.ISLIKE: "islike",
            }
    ...
    def _parse_range(self) -> ASTNode:
        ...
        if self._match(TokenType.RANGE_INCL):
            ...
            return Range(
                start=left, end=right, inclusive=True, line=tok.line, column=tok.column
            )
        ...
        if self._match(TokenType.RANGE):
            ...
            right = self._parse_bitwise_or()
            return Range(
                start=left, end=right, inclusive=False, line=tok.line, column=tok.column
            )
        ...
        while True:
            ...
            if self._check(TokenType.LPAREN) and not isinstance(expr, Literal):
                ...
                if self._check(TokenType.IDENTIFIER):
                    ...
                    expr = Member(
                        obj=expr, field=field_name, line=tok.line, column=tok.column
                    )
                elif self._check(
                    TokenType.STR,
                    TokenType.INT,
                    ...
                    if self._is_step_token():
                        ...
                        while self._is_step_token():
                            ...
                            if self._match(TokenType.LPAREN):
                                ...
                                self._consume(
                                    TokenType.RPAREN,
                                    f"Expected ')' after {step_name} arguments",
                                )
                        ...
                        raise SyntaxError(
                            f"Line {tok.line}: Expected field name after '.'"
                        )
                ...
                expr = StepChain(
                    source=expr, steps=steps, line=tok.line, column=tok.column
                )
    ...
    def _is_step_token(self) -> bool:
        ...
        return self._check(
            TokenType.STEP_FILTER,
            TokenType.STEP_SORT,
            TokenType.STEP_MAP,
            TokenType.STEP_TAKE,
            TokenType.STEP_DROP,
            TokenType.STEP_FIRST,
            TokenType.STEP_LAST,
            TokenType.STEP_REVERSE,
            TokenType.STEP_UNIQUE,
            TokenType.STEP_COUNT,
            TokenType.STEP_SUM,
            TokenType.STEP_AVG,
            TokenType.STEP_MIN,
            TokenType.STEP_MAX,
            TokenType.STEP_GROUP,
            TokenType.STEP_FLATTEN,
            TokenType.STEP_ZIP,
            TokenType.STEP_CHUNK,
        )
            ...
            if self._check(TokenType.RPAREN) or self._check(TokenType.IDENTIFIER):
                ...
                try:
                    ...
                    if self._match(TokenType.FAT_ARROW):
                        body = self._parse_expression()
                        return Lambda(
                            params=params, body=body, line=tok.line, column=tok.column
                        )
                except:
                    pass

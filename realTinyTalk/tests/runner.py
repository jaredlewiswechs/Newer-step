...
def run_test(
    name: str, code: str, expected: str, file: str, line: int, category: TestCategory
) -> TestResult:
...
def print_results(
    ...
    for suite in suites:
        ...
        print(
            f"{status} {cat_label:8} {suite.name}: {suite.passed}/{suite.total} ({suite.time_ms:.1f}ms)"
        )
    ...
    if all_failures:
        ...
        print("-" * 65)
        print(
            f"  FAILURES ({len(all_failures)} total, showing first {min(len(all_failures), max_failures)}):"
        )
        print()
...
Examples:
  ...
  python runner.py 09_strings.tt      # Run specific file
    ...
    parser.add_argument(
        "-c", "--code", action="store_true", help="Show code for failed tests"
    )
    parser.add_argument(
        "--filter", type=str, default=None, help="Filter tests by name pattern"
    )
    parser.add_argument(
        "--category",
        type=str,
    ...
    parser.add_argument(
        "--max-failures", type=int, default=10, help="Maximum failures to display"
    )

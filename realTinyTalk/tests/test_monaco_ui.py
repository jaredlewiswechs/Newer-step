...
class TestRunner:
    ...
    def run_code(self, code: str) -> dict:
        ...
        try:
            resp = requests.post(
                f"{self.base_url}/api/run", json={"code": code}, timeout=10
            )
            return resp.json()
                ...
                if tc.expected_contains:
                    for expected in tc.expected_contains:
                        if expected not in output:
                            errors.append(
                                f"Expected '{expected}' not in output: {output[:100]}"
                            )
        else:
...
def demo_transpile_output():
    ...
    for code, name in samples:
        ...
        try:
            js = requests.post(
                f"{BASE_URL}/api/transpile/js",
                json={"code": code, "include_runtime": False},
                timeout=5,
            ).json()
            py = requests.post(
                f"{BASE_URL}/api/transpile/python",
                json={"code": code, "include_runtime": False},
                timeout=5,
            ).json()

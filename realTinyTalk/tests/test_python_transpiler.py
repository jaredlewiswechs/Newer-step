...
def test_transpile(
    name: str, code: str, expected_contains: list = None, should_fail: bool = False
):
...
def run_all_tests():
    ...
    if test_transpile(
        "Logical and", "let x = true and false", ["x = (True and False)"]
    ):
        passed += 1
    else:
        failed += 1
    ...
    if test_transpile(
        "has array contains", "let arr = [1,2,3]\nlet x = arr has 2", ["tt.has(arr, 2)"]
    ):
        passed += 1
    else:
        failed += 1
    ...
    if test_transpile(
        "if statement", "if true { show(1) }", ["if True:", "tt.show(1)"]
    ):
        passed += 1
    else:
        failed += 1
    ...
    if test_transpile(
        "for loop",
        "for i in range(3) { show(i) }",
        ["for i in range(3):", "tt.show(i)"],
    ):
        passed += 1
    else:
        failed += 1
    ...
    if test_transpile(
        "Chained steps",
        "let nums = [5,2,8]\nshow(nums _sort _reverse _take(2))",
        [".sort()", ".reverse()", ".take(2)"],
    ):
        passed += 1
    else:
...
"""
    ...
    if test_transpile(
        "Syntax error: missing bracket", "let x = [1, 2, 3", should_fail=True
    ):
        passed += 1
    else:
        failed += 1
...
"""
    if test_transpile(
        ...
        nested_fn_code,
        ["def outer(a):", "def inner(b):", "return (b * 2)", "return (inner(a) + 1)"],
    ):
        passed += 1
    else:
        failed += 1
    ...
    if test_transpile(
        "Deeply chained steps",
        "let x = [1,2,3]._sort._reverse._take(2)._sum",
        ["x = sum(take(reverse(sort([1, 2, 3])), 2))"],
    ):
        passed += 1
    else:
        failed += 1
...
"""
    if test_transpile(
        ...
        complex_cf_code,
        ["for i in range(3):", "if ((i % 2) == 0):", "total = (total + i)"],
    ):
        passed += 1
    else:
        failed += 1

...
class PythonEmitter:
    ...
    def _emit_runtime_header(self):
        ...
        self._write_raw(
            '        """Check if obj contains item (list/dict) or has attribute."""'
        )
        ...
        self._write_raw(
            '        """Glob-style pattern matching (N* matches Newton)."""'
        )
        ...
        self._write_raw("            props = {")
        self._write_raw(
            '                "len": len(obj), "length": len(obj), "count": len(obj),'
        )
    ...
    def _emit_node(self, node: ASTNode) -> str:
        ...
        if ntype == NodeType.EXPR_STMT:
            return (
                self._emit_node(node.expression) if hasattr(node, "expression") else ""
            )

...
class JSEmitter:
    ...
    def _emit_runtime_header(self):
        ...
        self._write_raw(
            "  str: (x) => x === null ? 'null' : x === undefined ? 'null' : String(x),"
        )
    ...
    def _emit_for(self, node: ForStmt) -> str:
        ...
        if isinstance(node.iterable, Call) and isinstance(
            node.iterable.callee, Identifier
        ):
            if node.iterable.callee.name == "range":
                ...
                if len(args) == 1:
                    ...
                    if len(args) >= 3:
                        ...
                        lines.append(
                            f"for (let {var} = {start}; {var} < {end}; {var} += {step}) {{"
                        )
                    else:
                        lines.append(
                            f"for (let {var} = {start}; {var} < {end}; {var}++) {{"
                        )
            else:
    ...
    def _emit_class(self, node: StructDecl) -> str:
        ...
        if node.fields:
            params = ", ".join(
                f'{f[0]} = {self._emit_node(f[2]) if len(f) > 2 and f[2] else "null"}'

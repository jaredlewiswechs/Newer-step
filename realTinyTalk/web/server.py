    ...
    if scripts_dir.exists():
        for d in sorted(scripts_dir.iterdir()):
            if d.is_dir():
                meta = _read_meta(d)
                result.append(
                    {"name": d.name, "versions": len(meta.get("versions", []))}
                )
...
def get_script(name):
    ...
    return jsonify(
        {"name": name, "content": content, "versions": meta.get("versions", [])}
    )
    ...
    def collect_target_for_range(opcodes, target_lines, start, end):
        ...
        for tag, i1, i2, j1, j2 in opcodes:
            ...
            if overlap_end > overlap_start:
                ...
                out.extend(
                    target_lines[j1 + off : j1 + off + (overlap_end - overlap_start)]
                )
        return out
...
def merge_three_way(name):
    ...
    try:
        ...
        if which("git"):
            with tempfile.TemporaryDirectory() as td:
                ...
                proc = subprocess.run(
                    ["git", "merge-file", "-p", str(af), str(basef), str(bf)],
                    capture_output=True,
                    text=True,
                )
    ...
    return jsonify(
        {"merged": {"id": saved["id"], "conflicts": conflicts, "content": merged_text}}
    )
...
if __name__ == "__main__":
    ...
    if _port_in_use(HOST, PORT):
        print(
            f"Server appears to be already running on {HOST}:{PORT}. Not starting a new instance."
        )
    else:
        app.run(

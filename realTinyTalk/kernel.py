...
class Trace:
    ...
    @staticmethod
    def t(
        step: str, ok: bool, meta: Optional[Dict] = None, note: Optional[str] = None
    ) -> "Trace":
        ...
        try:
            ...
            trace.append(
                Trace.t(
                    "compile:parse",
                    True,
                    {"nodes": len(ast) if isinstance(ast, list) else 1},
                )
            )
        ...
        try:
            ...
            trace.append(
                Trace.t(
                    "exec:done",
                    True,
                    {"ops": runtime.op_count, "elapsed_ms": int(elapsed * 1000)},
                )
            )
...
class TinyTalkKernel:
    ...
    def run(self, source: str) -> Result:
        ...
        if isinstance(compile_result, Finfr):
            self.ledger.append(
                "finfr", {"source": source[:100]}, {"reason": compile_result.reason}
            )
        ...
        if isinstance(pre, Finfr):
            self.ledger.append(
                "finfr", {"source": source[:100]}, {"reason": pre.reason}
            )
        ...
        if isinstance(post, Finfr):
            self.ledger.append(
                "unverified", {"source": source[:100]}, {"result": str(result)}
            )
            return finfr(post.reason, trace)

        # 5) Ledger commit
        entry = self.ledger.append(
            "fin", {"source": source[:100]}, {"result": str(result)}
        )

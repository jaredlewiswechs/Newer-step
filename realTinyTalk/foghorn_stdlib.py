            ...
            Value.map_val(
                {
                    ...
                    "accepts": Value.list_val(
                        [Value.string_val(t.value) for t in s.accepts]
                    ),
                    "produces": Value.list_val(
                        [Value.string_val(t.value) for t in s.produces]
                    ),
                }
            )
            for s in services
...
def builtin_services_run(args: List[Value]) -> Value:
    ...
    service_name = (
        args[0].data if args[0].type == ValueType.STRING else str(args[0].data)
    )
        ...
        return Value.map_val(
            {
                ...
                "results": Value.list_val(
                    [foghorn_to_tinytalk(r) for r in result.results]
                ),
                "error": (
                    Value.string_val(result.error) if result.error else Value.null_val()
                ),
            }
        )
...
def builtin_link_new(args: List[Value]) -> Value:
    ...
    target_hash = (
        args[1].data if args[1].type == ValueType.STRING else str(args[1].data)
    )

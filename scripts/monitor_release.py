...
for i in range(TRIES):
    try:
        ...
        if r.status_code == 200:
            rel = r.json()
            print(f"Release found: id={rel.get('id')} name={rel.get('name')} draft={rel.get('draft')} prerelease={rel.get('prerelease')}")
    ...
    try:
        ...
        if wr.status_code == 200:
            ...
            tag_runs = [run for run in runs if run.get('head_branch') in (TAG, f"refs/tags/{TAG}") or (run.get('event') == 'push' and run.get('head_branch') == 'main' and any(TAG in c.get('message','') for c in [run.get('head_commit',{})]))]
            if tag_runs:
                for run in tag_runs[:5]:
                    print(f"Workflow run: id={run.get('id')} name={run.get('name')} status={run.get('status')} conclusion={run.get('conclusion')} url={run.get('html_url')}")
            else:
